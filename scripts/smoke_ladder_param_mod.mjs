// Ladder Cutoff ParamModEdge must force sample path (Bias→Range→frequency).
import fs from "fs";
...
function run(biasValue) {
  ...
  setSr(g, 48000);
  const hOsc = 0xd001 >>> 0;
  const hBias = 0xd002 >>> 0;
  const hRange = 0xd003 >>> 0;
  const hFilt = 0xd004 >>> 0;
  const hOut = 0xd005 >>> 0;
  ...
  if (add(g, hFilt, TYPE_LADDER)) throw new Error("ladder");
  if (add(g, hOut, TYPE_OUT)) throw new Error("out");
  connect(g, hOsc, PORT_MONO, hFilt, PORT_MONO);
  ...
  setParam(g, hRange, PARAM_OUT_LOW, 200);
  setParam(g, hRange, PARAM_OUT_HIGH, 4000);
  clearMods(g);
  if (addMod(g, hRange, PORT_MONO, hFilt, PARAM_FREQ)) throw new Error("mod");
...
if (!(Math.abs(low.rms - high.rms) > 0.005 || Math.abs(low.peak - high.peak) > 0.01)) {
  throw new Error(`ladder cutoff MOD inert ${JSON.stringify({ low, high })}`);
}
console.log("smoke_ladder_param_mod ok");
