// Headless: blit → output (440 Hz saw, level 1).
import fs from "fs";
...

{
  ...
  const hOsc = 0xb301 >>> 0;
  const hOut = 0xb302 >>> 0;
  if ((add(g, hOsc, TYPE_BLIT) | 0) !== 0) throw new Error("blit add");
  ...
  setParam(g, hOsc, PARAM_WAVEFORM, 0); // saw
  ...
  if (!(peak > 0.2 && peak <= 1.5)) throw new Error(`blit peak=${peak}`);
  if (!(rms > 0.05 && rms < 1.2)) throw new Error(`blit rms=${rms}`);
  console.log(`blit ok peak=${peak.toFixed(4)} rms=${rms.toFixed(4)}`);
}

if ((version() | 0) < 40) throw new Error(`graph version ${version()} expected >= 40`);
console.log(`smoke_graph_blit ok: version=${version() | 0}`);
