// Headless: sinc → output (440 Hz band-limited).
import fs from "fs";
...
const PARAM_MODE = 21; // bandLimit
const PARAM_STAGES = 22; // lobes
...

{
  ...
  const hOsc = 0xc801 >>> 0;
  const hOut = 0xc802 >>> 0;
  if ((add(g, hOsc, TYPE_SINC) | 0) !== 0) throw new Error("sinc add");
  ...
  setParam(g, hOsc, PARAM_PHASE, 0);
  setParam(g, hOsc, PARAM_STAGES, 4);
  ...
  if (!(peak > 0.2 && peak <= 1.0001)) throw new Error(`sinc peak=${peak}`);
  if (!(rms > 0.05)) throw new Error(`sinc rms=${rms}`);
  console.log(`sinc ok peak=${peak.toFixed(4)} rms=${rms.toFixed(4)}`);
}

if ((version() | 0) < 41) throw new Error(`graph version ${version()} expected >= 41`);
console.log(`smoke_graph_sinc ok: version=${version() | 0}`);
