// Headless: surgeOscillator → output (440 Hz saw, internal sync).
import fs from "fs";
...
const PARAM_WIDTH = 31; // syncFrequency
...

{
  ...
  const hOsc = 0xc401 >>> 0;
  const hOut = 0xc402 >>> 0;
  if ((add(g, hOsc, TYPE_SURGE) | 0) !== 0) throw new Error("surgeOscillator add");
  ...
  setParam(g, hOsc, PARAM_WIDTH, 110);
  ...
  if (!(peak > 0.2 && peak <= 1.5)) throw new Error(`surgeOscillator peak=${peak}`);
  if (!(rms > 0.05)) throw new Error(`surgeOscillator rms=${rms}`);
  console.log(`surgeOscillator ok peak=${peak.toFixed(4)} rms=${rms.toFixed(4)}`);
}

if ((version() | 0) < 41) throw new Error(`graph version ${version()} expected >= 41`);
console.log(`smoke_graph_surge_oscillator ok: version=${version() | 0}`);
