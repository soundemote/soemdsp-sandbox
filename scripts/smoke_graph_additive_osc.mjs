// Headless: additiveOsc → output (440 Hz sawtooth bank).
import fs from "fs";
...

{
  ...
  const hOsc = 0xc301 >>> 0;
  const hOut = 0xc302 >>> 0;
  if ((add(g, hOsc, TYPE_ADDITIVE) | 0) !== 0) throw new Error("additiveOsc add");
  ...
  setParam(g, hOsc, PARAM_STAGES, 32);
  ...
  if (!(peak > 0.05 && peak <= 1.0001)) throw new Error(`additiveOsc peak=${peak}`);
  if (!(rms > 0.01)) throw new Error(`additiveOsc rms=${rms}`);
  console.log(`additiveOsc ok peak=${peak.toFixed(4)} rms=${rms.toFixed(4)}`);
}

if ((version() | 0) < 41) throw new Error(`graph version ${version()} expected >= 41`);
console.log(`smoke_graph_additive_osc ok: version=${version() | 0}`);
