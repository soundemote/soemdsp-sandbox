...
function nodeGraphScientificIirClampOrder(order) {
  let o = Math.round(nodeGraphFiniteNumber(order, 4));
  if (o < 2) o = 2;
  if (o > 8) o = 8;
  if (o & 1) o += 1;
  return o;
...
function nodeGraphScientificIirChebyEps(rippleDb) {
  const r = Math.max(0.01, nodeGraphFiniteNumber(rippleDb, 0.01));
...
function nodeGraphScientificIirEllipticQ(order, i, rippleDb) {
  ...
  const boost = 1 + 0.35 * Math.max(0.1, nodeGraphFiniteNumber(rippleDb, 0.1));
...
function nodeGraphScientificIirDesignSection(mode, f0, Q, rate) {
  ...
  let f = Math.max(1e-9, Math.min(sr * 0.49, nodeGraphFiniteNumber(f0)));
...
function nodeGraphScientificIirDesign(kind, mode, order, freqHz, bandwidthOct, rippleDb, sampleRate) {
  const safeMode = Math.max(0, Math.min(3, Math.round(nodeGraphFiniteNumber(mode))));
  const rate = Math.max(1, nodeGraphFiniteNumber(sampleRate, 44100));
  const freq = Math.max(0, nodeGraphFiniteNumber(freqHz));
...
function nodeGraphScientificIirEnsure(state, kind, mode, order, freqHz, bandwidthOct, rippleDb, sampleRate) {
  const rate = Math.max(1, nodeGraphFiniteNumber(sampleRate, 44100));
  const safeKind = Math.max(0, Math.min(4, Math.round(nodeGraphFiniteNumber(kind))));
  const safeMode = Math.max(0, Math.min(3, Math.round(nodeGraphFiniteNumber(mode))));
  const safeOrder = nodeGraphScientificIirClampOrder(order);
  const freq = Math.max(0, nodeGraphFiniteNumber(freqHz));
  const bw = Math.max(0.05, nodeGraphFiniteNumber(bandwidthOct, 1));
  const ripple = Math.max(0.01, nodeGraphFiniteNumber(rippleDb, 0.01));
  if (
