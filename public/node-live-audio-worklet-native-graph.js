...
/** Param key → native Control id (for sample-accurate ParamModEdge). */
NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_KEY_IDS = Object.freeze({
  volumeDb: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_VOLUME_DB,
  pan: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_PAN,
  ...
  seed: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_SEED,
  offset: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_ATT_OFFSET,
  mean: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_ATT_OFFSET,
  pitch: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_ATT_OFFSET,
  lowpass: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_LPF_FREQUENCY,
  highpass: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_HPF_FREQUENCY,
  lpfFrequency: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_LPF_FREQUENCY,
  hpfFrequency: NodeLiveAudioProcessor.NATIVE_GRAPH_PARAM_HPF_FREQUENCY,
  ...
  // Hypersaw2 (and any module using these Control slots under local names).
...
NodeLiveAudioProcessor.prototype.mapNativeGraphSrcPortId = function mapNativeGraphSrcPortId(
  ...
  // Rare isIdle outs (envelopes, reverb, delay) → Meta Voice Idle.
  if (p === "isidle" || p === "idle") {
    return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_IS_IDLE;
  }
  ...
  if (p === "saw" || p === "mod l" || p === "lfo l" || p === "mod left" || p === "lfo left") {
    return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_SAW;
  }
  if (p === "ramp" || p === "mod r" || p === "lfo r" || p === "mod right" || p === "lfo right") {
  ...
  if (t === "basicShape") {
    if (p === "trisaw") return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_TRISAW;
    if (p === "center square") return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_CENTER_SQUARE;
    if (p === "__phase" || p === "phase01") {
      return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_PHASE01;
    }
  }
  ...
  if (t === "fractalBrownianNoise") {
    ...
    if (p === "out y raw") return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_RAMP;
    if (p === "out z raw") return NodeLiveAudioProcessor.NATIVE_GRAPH_PORT_SQUARE;
    ...
    try {
      ...
      const connectOne = (srcId, srcPort, dstId, dstPort) => {
        ...
        if (!feedHash) {
          ...
          // reads feed.hash (not feedHash). Without this, Keyboard Gate→ADSR
          // dies after surgical rewires while the Keyboard LCD still shows Gate.
          hostFeeders.push({
            hash: feedHash,
            sourceNode: srcId,
      ...
      for (const c of connections) {
        ...
        const dstPort = String(c?.destinationPort || "");
        // Same as full compile: walk 1D Phosphor Thru / observer outs to upstream.
        // Without this, surgical rewire Bias-feeds the monitor (silence).
...
NodeLiveAudioProcessor.prototype.patchHasMetamodule = function patchHasMetamodule() {
  if (this._patchHasMetamodule === true || this._patchHasMetamodule === false) {
    return this._patchHasMetamodule;
  }
  let has = false;
  for (const node of this.nodes.values()) {
    if (String(node?.type || "") === "metamodule") {
      has = true;
      break;
    }
  }
  this._patchHasMetamodule = has;
  return has;
};
 ...
 * No gold-bitmask OR into Voices. No /N amp.
 */
NodeLiveAudioProcessor.prototype.syncNativeMetaPolyphonyVoiceGates = function syncNativeMetaPolyphonyVoiceGates() {
  ...
  // Non-meta patches must not create a VoiceManager or walk MIDI/idle every quantum.
  const lanes = Array.isArray(this._nativeMetaVoiceLanes) ? this._nativeMetaVoiceLanes : [];
  if (!lanes.length && !this.patchHasMetamodule?.()) {
    return;
  }
  ...
  if (!this._metaVoicePrevMidiByMeta) {
    this._metaVoicePrevMidiByMeta = new Map();
  }
  ...
  const voiceHz = typeof polyphonyVoiceHz === "function"
    ? polyphonyVoiceHz
    : (midi, metaNode) => {
      if (!(midi >= 0)) return 0;
  ...
  // Meta playmode → VoiceManager PhonyMode / SlideMode.
  // 1 Mono → mono + never_slide
  // 2 Legato Ties → mono + allow_slide
  // 3 Legato Always → mono + always_slide
  // 4 Voices → poly + never_slide
  const applyPlaymode = (handle, playmode, voiceCount) => {
    const poly = playmode === 4 ? Math.max(1, Math.min(32, voiceCount)) : 1;
    native.soemdsp_voice_manager_set_polyphony?.(handle, poly);
    if (playmode === 4) {
      native.soemdsp_voice_manager_set_phony_mode?.(handle, 1); // poly
      native.soemdsp_voice_manager_set_slide_mode?.(handle, 0);
    } else {
      native.soemdsp_voice_manager_set_phony_mode?.(handle, 0); // mono
      const slide = playmode === 2 ? 1 : (playmode === 3 ? 2 : 0);
      native.soemdsp_voice_manager_set_slide_mode?.(handle, slide);
    }
    return poly;
  };
  ...
  const h = this.ensureVoiceManager?.() | 0;
  if (!(h > 0)) {
    // No VM — force Voice Gate feeders low so ADSR cannot stick open.
    for (let fi = 0; fi < busFeeders.length; fi += 1) {
      const feed = busFeeders[fi];
      if (!feed?.hash) continue;
      if (String(feed.kind || "") === "gate") {
  ...
  for (const [metaIdRaw, metaNode] of this.nodes) {
    ...
    // (Voice Gate → ADSR → Amp Curve → Amp). Direct amp mute caused a second
...
NodeLiveAudioProcessor.prototype.syncNativeGraphParams = function syncNativeGraphParams(_frames = 128) {
  ...
  const pushChanged = (hash, cache, key, paramId, value, node) => {
    ...
    // Dirty cache used to only gate set_param — we still paid full JS every quantum
    // × Voice Count clones (~200 pushChanged/quantum on the poly Hypersaw patch).
    const paramMods = this.modulationConnections?.get?.(
    ...
    // pushing a fake 0…1 domain was wrong: liveModActive (even at CV=0) then
    // clamped Frequency/etc into 0…1 and killed ghosts + audible MOD.
    ...
    } else if (cache[domainKey] == null) {
      cache[domainKey] = ""; // mark warm even when no domain range
    }
    ...
    if (discreteKey && !continuousCurveOverride && !continuousVoicesOverride) {
      ...
      if (cache[typeKey] == null) cache[typeKey] = -1;
  ...
  for (const [id, node] of this.nodes) {
    ...
    if (type === "inertialFilter") {
      // mode=smoothAttack (On=use Attack Hz; Off=instant rise).
      push("smoothAttack", P.NATIVE_GRAPH_PARAM_MODE, disc("smoothAttack", 1));
    ...
    if (type === "expAdsr" || type === "curveEnvelopeMod") {
    ...
    if (type === "wavetableAdsr") {
      // mode=shape; A/D/S/R/Level use control_audio so MOD is live per sample.
    ...
    if (type === "linearAttackRelease") {
      // timeNumerator=attack, timeDenominator=release, mode=inputMode, amplitude.
      push("attack", P.NATIVE_GRAPH_PARAM_TIME_NUMERATOR, cont("attack", 0.01));
      push("release", P.NATIVE_GRAPH_PARAM_TIME_DENOMINATOR, cont("release", 0.25));
      push("inputMode", P.NATIVE_GRAPH_PARAM_MODE, disc("inputMode", 0));
      push("amplitude", P.NATIVE_GRAPH_PARAM_AMPLITUDE, cont("amplitude", 1));
      continue;
    }
    if (type === "curveAttackRelease") {
      // timeDen=attack, offsetMs=release, shape=attackShape, center=releaseShape,
      // mode=inputMode, timingMode=updateOnTrigger, amplitude.
      push("updateOnTrigger", P.NATIVE_GRAPH_PARAM_TIMING_MODE, disc("updateOnTrigger", 0));
      push("inputMode", P.NATIVE_GRAPH_PARAM_MODE, disc("inputMode", 0));
      push("attack", P.NATIVE_GRAPH_PARAM_TIME_DENOMINATOR, cont("attack", 0.01));
      push("attackShape", P.NATIVE_GRAPH_PARAM_SHAPE, cont("attackShape", 0));
    ...
    if (type === "chaosfly") {
      push("outputMode", P.NATIVE_GRAPH_PARAM_MODE, disc("outputMode", 0));
      push("phase", P.NATIVE_GRAPH_PARAM_PHASE, cont("phase", 0));
      push("frequency", P.NATIVE_GRAPH_PARAM_FREQUENCY, cont("frequency", 55));
      push("pitch", P.NATIVE_GRAPH_PARAM_ATT_OFFSET, cont("pitch", 0));
      push("masterFm", P.NATIVE_GRAPH_PARAM_SHAPE, cont("masterFm", 0));
      // LP/HP = octave offsets from pitched master (Frequency × 2^Pitch).
      push("lowpass", P.NATIVE_GRAPH_PARAM_LPF_FREQUENCY, cont("lowpass", 6));
      push("highpass", P.NATIVE_GRAPH_PARAM_HPF_FREQUENCY, cont("highpass", -2));
      push("taps", P.NATIVE_GRAPH_PARAM_STAGES, disc("taps", 4));
      push("hpPosition", P.NATIVE_GRAPH_PARAM_OVERSAMPLE, disc("hpPosition", 0));
      push("osc1Detune", P.NATIVE_GRAPH_PARAM_WIDTH, cont("osc1Detune", 0));
      push("osc2Detune", P.NATIVE_GRAPH_PARAM_CENTER, cont("osc2Detune", 0));
      push("fm1", P.NATIVE_GRAPH_PARAM_IN_LOW, cont("fm1", 0));
      push("fm2", P.NATIVE_GRAPH_PARAM_IN_HIGH, cont("fm2", 0));
      push("pan", P.NATIVE_GRAPH_PARAM_PAN, cont("pan", 0));
      // Volume (key amplitude, 0…10) — master gain on Out/L/R and face X/Y/Z.
      {
        const ampRaw = Number(node?.params?.amplitude ?? node?.params?.volume);
        push(
          "amplitude",
    ...
    if (type === "mix4" || type === "mix" || type === "gainBiasMix") {
      ...
      // Bias removed from Mix4 face; keep native lanes at 0 for old patches.
    ...
    if (type === "mixStereo2") {
      push("volume1", P.NATIVE_GRAPH_PARAM_LANE_VOL1, cont("volume1", 0));
      push("volume2", P.NATIVE_GRAPH_PARAM_LANE_VOL2, cont("volume2", 0));
      // Silence unused pairs 3/4 (−140 dB).
      push("volume3", P.NATIVE_GRAPH_PARAM_LANE_VOL3, -140);
      push("volume4", P.NATIVE_GRAPH_PARAM_LANE_VOL4, -140);
      push("pan1", P.NATIVE_GRAPH_PARAM_LANE_BIAS1, cont("pan1", 0));
      push("pan2", P.NATIVE_GRAPH_PARAM_LANE_BIAS2, cont("pan2", 0));
      push("amplitude", P.NATIVE_GRAPH_PARAM_VOLUME_DB, cont("amplitude", 0));
      continue;
...
NodeLiveAudioProcessor.prototype.updateNativeAudioPlayerMeters = function updateNativeAudioPlayerMeters() {
  ...
  // Prefer a player that still has PCM (matches old evaluator primary).
  let primaryId = "";
  for (let i = 0; i < ids.length; i += 1) {
    ...
    let speed = readNum(node, "speed", 1);
    const speedKey = typeof this.inputKey === "function"
      ? this.inputKey(id, "Speed")
      : `${id}.Speed`;
    const speedConns = this.inputConnections?.get?.(speedKey);
    if (Array.isArray(speedConns) && speedConns.length) {
      let cv = 0;
      for (let c = 0; c < speedConns.length; c += 1) {
        const src = speedConns[c];
        if (!src) continue;
        const srcOut = this.nodeOutputs?.get?.(String(src.sourceNode || ""));
        if (!srcOut) continue;
        const sp = String(src.sourcePort || "");
        const v = Number(
          srcOut[sp]
          ?? srcOut.Out
          ?? srcOut.Bias
    ...
    if (!primaryId || hasPcm) {
      primaryId = id;
      this.audioPlayerMeterNodeId = id;
      this.audioPlayerMeterPhase = Number.isFinite(phase) ? phase : 0;
      this.audioPlayerMeterSpeed = speed;
      this.audioPlayerMeterReason = reason;
...
NodeLiveAudioProcessor.prototype.compileNativeGraphFromPlan = function compileNativeGraphFromPlan() {
  ...
  try {
    ...
    const nodes = [];
    const skipped = [];
    const addNativeNode = (id, type, params) => {
      const typeId = this.mapNativeGraphTypeId(type);
      ...
      if (rc !== 0) {
        const poolMsg = rc === -5
          ? `native instance pool exhausted for ${id} (${type}) — skipped`
          : `add_node failed (${rc}) for ${id} (${type}) — skipped`;
        skipped.push(`${type}:${rc}`);
        this.postNativeGraphStatus("warning", poolMsg);
        return false;
      }
    ...
    for (const [metaId, metaNode] of this.nodes) {
      ...
      for (const [childId, child] of this.nodes) {
        ...
        if (!t || META_VOICE_PORTAL_TYPES.has(t)) continue;
        // Only clone types that exist in the native audio graph.
      ...
      for (const childId of ownedVoiceIds) {
        ...
        if (!child) continue;
        const t = String(child?.type || "");
        const voiceIds = [childId];
        for (let v = 1; v < voiceCount; v += 1) {
    ...
    const voiceLaneByBase = new Map(metaVoiceLanes.map((l) => [l.baseId, l]));
    /** Expand authoring id → [id, id__v1, …] for any cloned owned module. */
    const expandVoiceIds = (id) => {
      const lane = voiceLaneByBase.get(String(id || ""));
      return lane ? lane.voiceIds : [String(id || "")];
    };
    ...
    for (const c of connections) {
      ...
      if (routeViaMetaAmp && attTypeId) {
        ...
        for (let r = 0; r < resolved.length; r += 1) {
          const item = resolved[r];
          // Poly Voices: every lane sums into Meta Out VCA (cloned → single).
          if (!connectExpanded(item.sourceNode, item.sourcePort, vcaId, "In")) {
            return false;
          }
        }
      ...
      for (let r = 0; r < resolved.length; r += 1) {
        const item = resolved[r];
        // Both cloned → pair lanes; cloned→single sum; single→cloned fan.
        if (!connectExpanded(item.sourceNode, item.sourcePort, dst, dstPort)) {
          return false;
        }
      }
    }
    ...
    for (let ci = 0; ci < connections.length; ci += 1) {
      ...
      const s = String(c?.sourceNode || "");
      const d = String(c?.destinationNode || "");
      if (s) wiredIds.add(s);
      if (d) wiredIds.add(d);
    }
    if (this.modulationConnections && typeof this.modulationConnections.forEach === "function") {
      this.modulationConnections.forEach((list) => {
        if (!Array.isArray(list)) return;
        for (let i = 0; i < list.length; i += 1) {
          const m = list[i];
          const s = String(m?.sourceNode || "");
          const d = String(m?.destinationNode || "");
          if (s) wiredIds.add(s);
          if (d) wiredIds.add(d);
        }
      });
...
NodeLiveAudioProcessor.prototype.nativeGraphPortNames = function nativeGraphPortNames(type, portId) {
  const P = NodeLiveAudioProcessor;
  if (portId === P.NATIVE_GRAPH_PORT_IS_IDLE) {
    return ["isIdle", "Idle"];
  }
  if (portId === P.NATIVE_GRAPH_PORT_MONO) {
    ...
    if (type === "sinCos") return ["sin", "Sin", "Out", "Mono"];
  ...
  if (portId === P.NATIVE_GRAPH_PORT_LEFT) {
    ...
    if (type === "sinCos") return ["cos", "Cos", "Left"];
  ...
  if (portId === P.NATIVE_GRAPH_PORT_RIGHT) {
    ...
    if (type === "transport") {
      return ["Trigger", "Trigger Sample", "Trig Sample"];
    }
    if (type === "reverbEffect" || type === "soemReverb" || type === "delayEffect") {
      return ["Right", "Mix Right", "Mix R", "Wet R", "Wet Right"];
    }
    if (type === "pingPongDelay") {
      return ["Right", "Mix R", "Mix Right"];
    }
    if (type === "noiseGenerator" || type === "cheapWalk" || type === "randomWalk") {
  ...
  if (portId === P.NATIVE_GRAPH_PORT_SAW) {
    ...
    if (type === "pingPongDelay") return ["LFO L", "Mod L", "Mod Left", "Saw"];
    return ["Saw"];
  }
  if (portId === P.NATIVE_GRAPH_PORT_RAMP) {
    ...
    if (type === "pingPongDelay") return ["LFO R", "Mod R", "Mod Right", "Ramp"];
    return ["Ramp"];
  }
  ...
  if (portId === P.NATIVE_GRAPH_PORT_TRISAW) {
    if (type === "basicShape") return ["Trisaw"];
    return [];
  }
  if (portId === P.NATIVE_GRAPH_PORT_CENTER_SQUARE) {
    if (type === "basicShape") return ["Center Square"];
    return [];
  }
...
NodeLiveAudioProcessor.prototype.publishNativeGraphScopeTaps = function publishNativeGraphScopeTaps(
  ...
  const publishOneNativeOut = (id, type) => {
    ...
    let any = false;
    const ports = type === "basicShape"
      ? audioPorts.concat(
        P.NATIVE_GRAPH_PORT_TRISAW,
  ...
  // Meta Voices clones (__v1…) are not in this.nodes — publish isIdle/Out for clean().
  const voiceLanesPub = Array.isArray(this._nativeMetaVoiceLanes) ? this._nativeMetaVoiceLanes : [];
  for (let li = 0; li < voiceLanesPub.length; li += 1) {
    const lane = voiceLanesPub[li];
    const t = String(lane.type || "");
    const ids = lane.voiceIds || [];
    for (let v = 1; v < ids.length; v += 1) {
      publishOneNativeOut(ids[v], t);
    }
  }
  ...
  if (Array.isArray(this.compiledScopeNodes)) {
    for (let i = 0; i < this.compiledScopeNodes.length; i += 1) {
      ...
      // Unwired leftover modules (e.g. Hypersaw sitting on the canvas) skip
      // audio-rate face rings — native DSP is also skipped for those.
      ...
      const visualKeys = this.visualInputBuffers;
      const hasVisualPorts = visualNodeIds instanceof Set
        ? visualNodeIds.has(String(nodeId))
        : false;
      for (let bi = 0; bi < bindings.length; bi += 1) {
        const binding = bindings[bi];
        const portIds = new Array(binding.names.length);
        for (let ni = 0; ni < binding.names.length; ni += 1) {
          portIds[ni] = `${nodeId}:${binding.names[ni]}`;
        }
        binding.portIds = portIds;
      }
      for (let frame = 0; frame < frames; frame += hop) {
        ...
        for (let bi = 0; bi < bindings.length; bi += 1) {
          ...
          for (let ni = 0; ni < binding.names.length; ni += 1) {
            const portId = binding.portIds[ni];
            if (visualKeys?.has?.(portId)) continue;
            this.appendScopeBufferSample?.(portId, sample);
          }
        }
        if (!hasVisualPorts && gotAgg) {
          this.appendScopeBufferSample?.(nodeId, aggregate);
        }
      }
    }
  }
...
NodeLiveAudioProcessor.prototype.processNativeGraphQuantum = function processNativeGraphQuantum(
  ...
  // Music Player HUD / waveform scroll read sampleRuntimeStatus from these
