...
function nodeGraphHypersawSample(state, options = {}) {
  ...
  const lastFrac = wasm.soemdsp_hypersaw_voice_last_frac
    ? nodeGraphFiniteNumber(wasm.soemdsp_hypersaw_voice_last_frac(state.nativeHandle))
    : 0;
  ...
  for (let i = 0; i < n; i++) {
    voicePhases[i] = wasm.soemdsp_hypersaw_voice_phase
      ? nodeGraphFiniteNumber(wasm.soemdsp_hypersaw_voice_phase(state.nativeHandle, i))
      : 0;
    const isCenter = i === 0;
  ...
  return {
    Left: nodeGraphFiniteNumber(wasm.soemdsp_hypersaw_left(state.nativeHandle)),
    Right: nodeGraphFiniteNumber(wasm.soemdsp_hypersaw_right(state.nativeHandle)),
    voicePhases,
    voiceAmplitudes,
    voicePans,
  };
